pub mod barcode {
	...
	pub mod prelude {
		pub use super::{BarcodeDetectorTrait, BarcodeDetectorTraitConst};
	}
	...
	impl TryFrom<i32> for BarcodeType {
		...
		fn try_from(value: i32) -> Result<Self, Self::Error> {
			match value {
				...
				3 => Ok(Self::UPC_A),
				4 => Ok(Self::UPC_E),
				5 => Ok(Self::UPC_EAN_EXTENSION),
				_ => Err(crate::Error::new(crate::core::StsBadArg, format!("Value: {value} is not valid for enum: crate::barcode::BarcodeType"))),
			}
		}
	}
	...
	impl BarcodeDetector {
		...
		/// * model_path: ""
		#[inline]
		pub fn new_def() -> Result<crate::barcode::BarcodeDetector> {
			return_send!(via ocvrs_return);
	...
	pub trait BarcodeDetectorTraitConst {
		...
		#[inline]
		fn decode(&self, img: &impl ToInputArray, points: &impl ToInputArray, decoded_info: &mut core::Vector<String>, decoded_type: &mut core::Vector<crate::barcode::BarcodeType>) -> Result<bool> {
			input_array_arg!(img);
		...
		/// * points: noArray()
		#[inline]
		fn detect_and_decode(&self, img: &impl ToInputArray, decoded_info: &mut core::Vector<String>, decoded_type: &mut core::Vector<crate::barcode::BarcodeType>, points: &mut impl ToOutputArray) -> Result<bool> {
			input_array_arg!(img);
			output_array_arg!(points);
			return_send!(via ocvrs_return);
		...
		///
		/// ## Note
		/// This alternative version of [BarcodeDetectorTraitConst::detect_and_decode] function uses the following default values for its arguments:
		/// * points: noArray()
		#[inline]
		fn detect_and_decode_def(&self, img: &impl ToInputArray, decoded_info: &mut core::Vector<String>, decoded_type: &mut core::Vector<crate::barcode::BarcodeType>) -> Result<bool> {
			input_array_arg!(img);
			return_send!(via ocvrs_return);
			unsafe { sys::cv_barcode_BarcodeDetector_detectAndDecode_const_const__InputArrayR_vectorLstringGR_vectorLBarcodeTypeGR(self.as_raw_BarcodeDetector(), img.as_raw__InputArray(), decoded_info.as_raw_mut_VectorOfString(), decoded_type.as_raw_mut_VectorOfBarcodeType(), ocvrs_return.as_mut_ptr()) };
			return_receive!(ocvrs_return => ret);
			let ret = ret.into_result()?;
			Ok(ret)
		}
	...
	boxed_ref! { BarcodeDetector, crate::barcode::BarcodeDetectorTraitConst, as_raw_BarcodeDetector, crate::barcode::BarcodeDetectorTrait, as_raw_mut_BarcodeDetector }
