...
fn main() -> Result<()> {
    ...
    if args.len() < 4 {
        eprintln!("Usage: {} <input_video> <csv_data> <output_video>", args[0]);
        std::process::exit(1);
    }
    let video_path = &args[1];
    let csv_path = &args[2];
    let output_path = &args[3]; // new argument
    ...
    let mut writer = videoio::VideoWriter::new(
        output_path, // use provided filename
        fourcc,
        fps,
    ...
    loop {
        ...
        for (i, &(tx, ty)) in trail.iter().enumerate() {
            ...
            let radius = (5.0 + alpha * 12.0) as i32;
            imgproc::circle(
                &mut canvas,
    ...
    writer.release()?;
    cap.release()?;
    println!("Overlay video written to {}", output_path);
    Ok(())
}
